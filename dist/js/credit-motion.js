...
export function bindCreditMotion(container=document) {
 container.querySelectorAll('[data-credit-motion]').forEach(root=>{
  ...
  const status=(step,copy)=>{root.dataset.creditStep=step;root.querySelector('.credit-status').textContent=copy;};
  function sync(){
  ...
  async function score(phase){
   status('matrix','Multiply corresponding rows, then collect each chunk’s score.');
   ...
   status('score-complete','Chunk scores are ready to become credit weights.');
  }
  ...
  async function allocate(phase){
   status('reveal','Turn the completed scores into action-level credit.');route();
   ...
   status('weights','Transform scores into weights at the same time positions.');
   ...
   status('normalize','Normalize across all chunks while preserving their relative weights.');
   await motion(phase.querySelector('.credit-bars-58'),[{opacity:.6},{opacity:1}],650);
   status('dense','Apply each weight to the matching advantage chunk.');
   await Promise.all([
   ...
   status('update','Dense credit guides the policy update.');
   await motion(root.querySelector('.credit-update strong'),[{opacity:.4},{opacity:1}],800);
  }
  async function pump(){
   ...
   if(stopped){sync();return;}next++;if(next===2){root.dataset.creditMode='complete';status('complete','Dense credit guides the policy update.');}sync();void pump();
  }
  ...
  function finishStatic(){stopped=true;observer.disconnect();cleanup();next=2;root.dataset.creditMode='complete';phases.forEach(p=>p.dataset.state='done');status('complete','Dense credit guides the policy update.');sync();}
