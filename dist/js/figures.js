...
export function credit(g){const film=(ids,failure=false)=>`<div class="film-row">${ids.map((n,i)=>`<div class="film-cell ${failure&&i===4?'error':''}"><img src="assets/photo-${n}.webp" alt="${failure?'Failed student':'Successful teacher'}, observation ${i+1} of 8${failure&&i===4?'; annotated empty-space grasp':''}" loading="lazy" width="128" height="128"></div>`).join('')}</div>`;return `<div class="credit-layout"><div>${matrixFigure(g['25'],'Illustrative cost matrix between the teacher and student')}<p class="matrix-name">Cost matrix</p></div><div><div class="film-label"><span>Successful teacher</span><span>Time →</span></div>${film([27,77,78,79,80,81,82,83])}<div class="credit-profile">${svg(820,137,text(10,20,'Student credit weight',14)+barsMarkup(g['58'],0,28,820,83)+arrow(0,114,820,114)+text(820,135,'t',12,C.muted,'end'),'Student credit weights, with higher weights toward the end of the illustrated trajectory.')}</div><div class="film-label failure"><span>Failed student</span><span>× Annotated failed grasp</span></div>${film([69,70,71,72,73,74,75,76],true)}</div></div>`;}

export const scenePhotos=[{id:67,label:'(a) Scene',alt:'UR5 scene with base and wrist cameras and five manipulation tasks'},{id:65,label:'(b) Stack blocks',alt:'Robot stacks foam blocks'},{id:63,label:'(c) Insert battery',alt:'Robot inserts a battery into a computer mouse'},{id:66,label:'(d) Solder component',alt:'Robot solders an electronic component'},{id:64,label:'(e) Drive screw',alt:'Robot drives a screw into an electronic enclosure'},{id:68,label:'(f) Pop the top',alt:'Robot removes a bottle cap'}];
export function scenes(){
 ...
 return `<div class="scene-grid">${scenePhotos.map((p,i)=>{
  const body=`<span class="scene-frame"><img src="assets/photo-${p.id}.webp" alt="${p.alt}" loading="lazy" width="700" height="620"></span><span class="scene-caption">${p.label}</span>`;
